...
// Pagamentos (PRD-11): PIX payout of the held BRL balance to a saved payee. This folder route
// takes precedence over the [section] "Em breve" placeholder. Balance + payees are real.
export default async function PayoutsPage() {
  ...
  const pixPayees = beneficiaries.filter((b) => b.rail === "pix" && b.status === "active");
    ...
    <div className="mx-auto max-w-md space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-warm-400">
          Envie seu saldo em reais via PIX para um beneficiário salvo.
        </p>
      </div>
      {pixPayees.length === 0 ? (
        <div className="rounded-[22px] bg-ink-800 p-6 text-center ring-1 ring-foreground/10">
          ...
          <h2 className="mt-3 font-display text-lg font-bold">Nenhum beneficiário PIX</h2>
          <p className="mt-2 text-sm leading-relaxed text-warm-300">
            Para enviar um pagamento, cadastre primeiro um beneficiário com chave PIX.
          </p>
          <Link
        ...
        <PayoutForm balances={balances} payees={pixPayees} />
      )}
    </div>
  );
}
