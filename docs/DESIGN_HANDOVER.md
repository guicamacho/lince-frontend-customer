...
States: **Active** `bg-ink-700 text-gold-500` · **Inactive** `text-warm-300` · **Inactive hover** `hover:bg-ink-700 hover:text-warm-100`. Active rule: `/app` exact match; others `pathname.startsWith(href)`. Props: none.
...
| `rfi_required` | 2 | 67% | gold | `Onboarding / 05 RFI Required` (caption "Documentos adicionais necessários") | "Precisamos de mais informações" / additional info requested | none (loops to step 2, not terminal) |
...
Gate: no `userId` → `/`; `state !== "active"` → `/onboarding` (single hard gate). Frame **`App / Shell (Top-bar + Sidebar + Main)`**: outer `flex min-h-screen flex-col` → `<TopBar/>` → row `flex flex-1` [ `<Sidebar/>` + `<main className="flex-1 p-8">` (32px pad) ].
- **`App / Home`** (`/app`): `space-y-2` → `h1 font-display text-2xl` "Início"; `p text-warm-300` "Bem-vindo à Lince Finance. Sua conta está ativa."; `p text-sm text-warm-500` "As funcionalidades (saldos, pagamentos, transações) chegam em breve."
- **`App / Section (Placeholder)`** (`/app/[section]`, one variant per section): `space-y-2` → `h1 font-display text-2xl` = title from map; `p text-warm-500` "Em construção." Title map: balances→"Saldos", pay→"Pagar / Enviar", transactions→"Transações", beneficiaries→"Beneficiários", team→"Equipe", settings→"Configurações"; unknown→"Página".
...
- **"Powered by Avenia" disclosure (PRD-required, currently MISSING).** Avenia is the regulated provider of record (onboarding copy: KYB "conduzida pela Didit e enviada à Avenia"). A "Powered by Avenia" disclosure is required by the PRD but is **not rendered anywhere in the code today**. Design a persistent disclosure slot — recommended in the auth left-column footer (below the form) and in the app footer/settings — so engineering can wire it without relayout. Keep it sentence-case, warm-400, `text-sm`/`text-xs`.
